...
class Base_VAE:
    ...
    def save(self, name):
        with open("training_outputs/{}_param.pkl".format(name), "wb") as f:
            d = (
                self.batch_size,
                self.L,
                self.q,
    ...
    def load(self, name):
        with open("training_outputs/{}_param.pkl".format(name), "rb") as f:
            d = pickle.load(f)
    ...
    def train(
        ...
        if name is not None:
            callbacks.append(CSVLogger("training_outputs/{}_train_log.csv".format(name)))
...
def main_plot_latent(name, args):
    ...
    plt.savefig("training_plots/LatentTraining_1d_{}.png".format(name))
    plt.close()
    ...
    plt.savefig("training_plots/LatentTraining_{}.png".format(name))
    plt.close()
